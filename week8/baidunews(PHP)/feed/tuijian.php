...
//“推荐”内容显示在手机页面的脚本
	...
	//定义查询：
	$query = "SELECT displaytype, tagtype, tagtext, image1_src, image2_src, image3_src, title, discribe, href, add_time FROM tuijian ORDER BY add_time DESC limit 0, $limit";
	...
	if ($r = mysql_query($query, $dbc)) {
		...
		while ($row = mysql_fetch_array($r)) {
        	...
        	if (!empty($row['tagtype'])){
        		print "			   <b class=\"tip-hot tip-fillred\">{$row['tagtext']}</b>";
        	}
